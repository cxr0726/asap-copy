...
class LeggedRobotBase(BaseTask):
    ...
    def _init_buffers(self):
        ...
        self.last_last_actions = torch.zeros(self.num_envs, self.dim_actions, dtype=torch.float, device=self.device,
                                        requires_grad=False)
    ...
    def _reset_buffers_callback(self, env_ids, target_buf=None):
        if target_buf is not None:
            ...
            self.last_last_actions[env_ids] = target_buf["last_last_actions"].to(self.last_actions.dtype)
    ...
    def _reward_penalty_ankle_action(self):

...
from humanoidverse.agents.modules.ppo_modules import PPOActorFixSigma,PPOActor,Distill_PPOActor
...
class DAgger(BaseAlgo):
    ...
    def _init_config(self):
        ...
        self.teacher_ckpt = self.config.teacher_ckpt
    ...
    def setup(self):
        ...
        self.load_teacher(self.teacher_ckpt)
        self.gt_actor.eval()

        self.actor = Distill_PPOActor(
            obs_dim_dict=self.algo_obs_dim_dict,
            module_config_dict=self.config.module_dict.distill_actor,
            num_actions=self.num_act,
            init_noise_std=self.config.init_noise_std
        ).to(self.device)
    ...
    def load_teacher(self, ckpt_path):
        if ckpt_path is not None:
            ...
            self.gt_actor.load_state_dict(loaded_dict["actor_model_state_dict"])
    ...
    def _rollout_step(self, obs_dict):
        with torch.inference_mode():
            for i in range(self.num_steps_per_env):
                ...
                actions = self.actor.act(obs_dict["distill_actor_obs"]).detach()
                ...
                gt_actions = self.gt_actor.act_inference(obs_dict["actor_obs"]).detach()
                gt_action_mean = gt_actions.clone()#self.gt_actor.action_mean.detach()
    ...
    def _training_step(self):
        ...
        for obs_dict in generator:
            ...
            mu_batch= self.actor.act_inference(obs_dict["distill_actor_obs"])
    ...
    def _pre_eval_env_step(self, actor_state: dict):
       # print()
        actions = self.eval_policy(actor_state["obs"]["distill_actor_obs"])
